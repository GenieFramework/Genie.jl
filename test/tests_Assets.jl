@safetestset "Assets functionality" begin
  ...
  @safetestset "Embedded assets" begin
    ...
    @test channels_support() == "<script src=\"/genie.jl/master/assets/js/$(hash(Genie.config.webchannels_default_route))/channels.js\"></script>"
    @test Genie.Router.routes()[1].path == "/genie.jl/master/assets/js/$(hash(Genie.config.webchannels_default_route))/channels.js"
