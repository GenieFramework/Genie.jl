@safetestset "HTML attributes rendering" begin
  ...
  <span v-if="seen">Now you see me</span>
    ...
    @test String(r.body) == """<html><head></head><body><div id="app-6"><p>{{ message }}</p><input v-model="message"$(Genie.config.html_parser_close_tag)></div></body></html>"""
    ...
    @test String(r.body) == """<html><head></head><body><img v-bind:src="submission.submissionImage"$(Genie.config.html_parser_close_tag)></body></html>"""

    r = html("""<img :src="submission.submissionImage" />""", forceparse = true)
    @test String(r.body) == """<html><head></head><body><img :src="submission.submissionImage"$(Genie.config.html_parser_close_tag)></body></html>"""
  end;
